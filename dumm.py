...
for batch in test_loader:
    logger.info(f"Test Batch: {batch}")
    break
...
for i, data in enumerate(tqdm(test_loader)):
    ...
    print(f"real_A: {real_A.shape}, real_B: {real_B.shape}")
    ...
    print(f"real_A: {real_A.shape}, real_B: {real_B.shape}")
    # real_A = real_A.view(1, 3, 64, 64)  # Reshape real_A to match [batch_size, channels, height, width]
